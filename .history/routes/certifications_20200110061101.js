 ...
 * @returns {undefined}
 */
module.exports.getCertification = async(request, response, next) => {
    // Get all the entries of content type course
    let getCertifications = Array[];
    let categories = []
    let certifications = await getCertification(response.locals.currentLocale.code, response.locals.currentApi.id)
        // Attach entry state flags when using preview API
    if (shouldAttachEntryState(response)) {
        certifications = await attachEntryStateToCourses(certifications)
    }

    categories = await getCategories(response.locals.currentLocale.code, response.locals.currentApi.id)
    response.render('courses', {
        title: `${translate('allCoursesLabel', response.locals.currentLocale.code)} (${certifications.length})`,
        categories,
        certifications
    })
}
